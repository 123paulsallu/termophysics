...
<div class="wrapper">
	...
	<aside class="main-sidebar sidebar-dark-primary elevation-4">
		...
		<div class="sidebar-user-panel text-center py-3" style="background:#003366;color:#BD8C2A;">
			<?php if (!isset($_SESSION['profile_photo'])) { $userRes = $conn->query("SELECT profile_photo FROM users WHERE id = {$_SESSION['user_id']}"); if ($userRes && $userRes->num_rows > 0) { $userData = $userRes->fetch_assoc(); $_SESSION['profile_photo'] = $userData['profile_photo']; } } ?>
			<?php if (!empty($_SESSION['profile_photo']) && file_exists($_SESSION['profile_photo'])): ?>
				<a href="settings.php"><img src="<?php echo htmlspecialchars($_SESSION['profile_photo']); ?>" alt="Profile" style="width:60px; height:60px; border-radius:50%; object-fit:cover; margin-bottom:10px; cursor:pointer;"></a>
			<?php else: ?>
				<a href="settings.php" style="display:block; margin-bottom:10px;"><div style="width:60px; height:60px; border-radius:50%; background:#BD8C2A; margin:0 auto; display:flex; align-items:center; justify-content:center; cursor:pointer;"><i class="fas fa-user" style="font-size:30px; color:#003366;"></i></div></a>
			<?php endif; ?>
			<div style="font-size:1.1em;font-weight:bold;"><?php echo htmlspecialchars(isset($_SESSION['username']) ? $_SESSION['username'] : ''); ?></div>
